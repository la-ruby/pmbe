  ...
  document.querySelectorAll('.product-tile__image-link:not(.pmbe)').forEach(function(item) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4) {
        ...
        elemDiv.style.cssText = 'font-size: 80%; position:absolute;top:0px;left:1px;width:100%;height:100%;opacity:1;z-index:1000;background-color:rgba(0,0,0,0.5);color:white';
        elemDiv.innerHTML = '<pre>' + JSON.stringify( JSON.parse(xhr.responseText), null, 2) + '</pre>'    '<br>' + xhr.responseText;
